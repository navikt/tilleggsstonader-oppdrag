...
import no.nav.tilleggsstonader.oppdrag.avstemming.konsistens.JaxbKonsistensavstemming
...
class AvstemmingSender(
    ...
    fun sendKonsistensAvstemming(avstemmingsdata: Konsistensavstemmingsdata) {
        val konsistensavstemmingRequest = SendAsynkronKonsistensavstemmingsdata().apply {
            request =
                SendAsynkronKonsistensavstemmingsdataRequest().apply { konsistensavstemmingsdata = avstemmingsdata }
        }
    ...
    companion object {
        val LOG = LoggerFactory.getLogger(AvstemmingSender::class.java)
    }
}
